# Лучшие практики создания запросов к OpenAI API
...
## Содержание
1. [Как работает создание запросов](#как-работает-создание-запросов)
2. [Правила и примеры](#правила-и-примеры)
   1. [Используйте последнюю модель](#1-используйте-последнюю-модель)
   2. [Поместите инструкции в начале запроса](#2-поместите-инструкции-в-начале-запроса)
   ...
   5. [Начинайте с нулевого примера (zero-shot), затем несколько примеров (few-shot)](#5-начинайте-с-нулевого-примера-zero-shot-затем-несколько-примеров-few-shot)
...
## Как работает создание запросов
...
[Официальное руководство по созданию запросов от OpenAI](https://platform.openai.com/docs/guides/prompt-engineering) обычно является лучшим местом для начала при поиске советов по созданию запросов.
...
Для лучших результатов мы обычно рекомендуем использовать последние, наиболее мощные модели. Новые модели, как правило, легче настраивать с помощью запросов.

### 2. Поместите инструкции в начале запроса
...
### 5. Начинайте с нулевого примера (zero-shot), затем несколько примеров (few-shot)
...
**Настройка (fine-tune) ✅:** Смотрите лучшие практики настройки здесь.
